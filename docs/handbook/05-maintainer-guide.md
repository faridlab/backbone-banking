...
metaphor make entity Vendor --module banking
#    …or copy an existing model (e.g. schema/models/bank_account.model.yaml) → vendor.model.yaml
#    and edit it, then add `- vendor.model.yaml` under `imports:` in schema/models/index.model.yaml.
...
metaphor schema schema generate banking --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor banking
...
> `banking` is your module name (auto-detected from the current directory when omitted).
...
### Step 6 in detail — wire the service into `BankingModule`

Generation does **not** edit the composition root for you. The root is `BankingModule` in
[`src/lib.rs`](../../src/lib.rs); follow the existing per-entity pattern (e.g. `bank_account_service`)
exactly:
...
// …return it in the BankingModule { … } literal…

// then expose it: for reads, add a create_vendor_read_routes(...) merge; for the full
// (unguarded) surface it is already covered by all_crud_routes().
```

Then re-export it in [`src/lib.rs`](../../src/lib.rs) alongside the other
`pub use application::service::…Service;` lines.

> **The banking write model is different.** The hand-written core
> (`banking_write_service.rs`, `banking_gl.rs`, `banking_events.rs`, `guarded_routes.rs`) is
> `user_owned` — it is *not* wired through the generated composition root and is never touched by
> regeneration. Edit those files directly; you don't need CUSTOM markers inside them.

## Changing an existing entity

1. Edit the field in `schema/models/<entity>.model.yaml` (the SSoT — never the generated struct).
2. `metaphor schema schema validate banking`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> banking` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration banking --database-url …`).
4. Regenerate code: `metaphor schema schema generate banking --target all --force`.
