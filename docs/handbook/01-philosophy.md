...
## This module's north star — banking

`backbone-banking` is the **cash & bank reconciliation** context: the truth-source between the
ledger of record and the outside-world cash position. Its worldview is narrower and sharper than the
generic module's:

- **Money is a fact, not a rounding.** Amounts are exact decimals (`@precision(18,2)`, IDR, half-up).
  Every clearing post is **balanced-or-refuse** — banking will not emit an unbalanced `AccountingPost`.
- **Banking posts one leg and owns no one else's masters.** It moves value `clearing → bank` (the
  bank-side leg) and reads **no** payment/billing table — matching is done against candidates the
  composition supplies. That isolation is a feature: the module deploys independently.
- **A control that can lie is worse than no control.** Reconciliation is three-state (`open` /
  `balanced` / `closed`): a session may not sign off "the bank agrees with our books" while lines
  sit unreconciled. Clearing is bounded twice so the same receipt can't be cleared into existence.
- **Prove it, don't assert it.** Every rule above has a golden case
  ([`golden-cases.md`](../business-flows/golden-cases.md)) and, for the cross-module cash loop, an
  end-to-end seam test. The docs are downstream of tests that are downstream of the schema.

The generic module trades boilerplate for consistency; banking spends the freed attention on the
5% that is genuinely a financial control. See [ADR-0004](adr/adr-0004-banking-boundary-and-clearing-seam.md).
