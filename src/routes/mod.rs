...
/// Create stateless routes for the Banking module.
///
...
///
/// ```ignore
/// let module = BankingModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::BankingModule) -> Router<()> {
    Router::new()
        .merge(create_bank_routes(module.bank_service.clone()))
        .merge(create_bank_account_routes(module.bank_account_service.clone()))
        .merge(create_bank_clearance_routes(module.bank_clearance_service.clone()))
        ...
        .merge(create_reconcile_preset_routes(module.reconcile_preset_service.clone()))
}
...
pub fn create_readonly_banking_routes(module: &crate::BankingModule) -> Router<()> {
    Router::new()
        ...
        .merge(create_reconcile_preset_read_routes(module.reconcile_preset_service.clone()))
}

/// Get all routes (stateless) for the Banking module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::BankingModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Banking module.
///
...
pub fn create_combined_routes(module: &crate::BankingModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Banking module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::BankingModule) -> Router<AppState> {
    Router::new()
