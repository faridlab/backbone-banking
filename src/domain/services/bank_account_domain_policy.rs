//! Domain policy for BankAccount aggregate
//!
...
/// Domain policy for BankAccount — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<BankAccount>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BankAccountDomainPolicy = PermitAllPolicy<BankAccount>;
