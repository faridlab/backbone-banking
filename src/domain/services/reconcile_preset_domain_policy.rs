//! Domain policy for ReconcilePreset aggregate
//!
...
/// Domain policy for ReconcilePreset — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ReconcilePreset>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ReconcilePresetDomainPolicy = PermitAllPolicy<ReconcilePreset>;
