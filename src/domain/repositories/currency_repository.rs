//! Repository trait for Currency aggregate
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! This trait defines the repository contract for the Currency aggregate.
//! Implementation is in the infrastructure layer.

use async_trait::async_trait;
use anyhow::Result;
use uuid::Uuid;

use crate::domain::entity::{Currency, CurrencyStatus};

/// Pagination parameters for list queries
#[derive(Debug, Clone, Default)]
pub struct CurrencyPaginationParams {
    pub page: u32,
    pub per_page: u32,
}

impl CurrencyPaginationParams {
    pub fn new(page: u32, per_page: u32) -> Self {
        Self { page, per_page }
    }

    pub fn offset(&self) -> u64 {
        ((self.page.saturating_sub(1)) * self.per_page) as u64
    }

    pub fn limit(&self) -> u64 {
        self.per_page as u64
    }
}

/// Paginated result for list queries
#[derive(Debug, Clone)]
pub struct CurrencyPaginatedResult {
    pub data: Vec<Currency>,
    pub total: u64,
    pub page: u32,
    pub per_page: u32,
}

/// Filter parameters for list queries
#[derive(Debug, Clone, Default)]
pub struct CurrencyFilter {
    pub company_id: Option<Uuid>,
    pub code: Option<String>,
    pub name: Option<String>,
    pub symbol: Option<String>,
    pub is_base: Option<bool>,
    pub status: Option<CurrencyStatus>,
}

impl CurrencyFilter {
    /// Check if any filter is set
    pub fn has_filters(&self) -> bool {
        self.company_id.is_some() || self.code.is_some() || self.name.is_some() || self.symbol.is_some() || self.is_base.is_some() || self.status.is_some()
    }
}

/// Repository trait for Currency aggregate
///
/// This trait defines the persistence contract for Currency entities.
/// Implementations should be in the infrastructure layer.
#[async_trait]
pub trait CurrencyRepository: Send + Sync {

    // =========================================================================
    // Core CRUD Operations
    // =========================================================================

    /// Save a new currency entity
    async fn save(&self, entity: &Currency) -> Result<Currency>;

    /// Find currency by ID
    async fn find_by_id(&self, id: &str) -> Result<Option<Currency>>;

    /// Find all currency entities
    async fn find_all(&self) -> Result<Vec<Currency>>;

    /// Update currency by ID
    async fn update(&self, id: &str, entity: &Currency) -> Result<Option<Currency>>;

    /// Delete currency by ID
    async fn delete(&self, id: &str) -> Result<bool>;

    // =========================================================================
    // Pagination
    // =========================================================================

    /// List currency with pagination
    async fn list(&self, params: CurrencyPaginationParams) -> Result<CurrencyPaginatedResult>;

    /// List currency with pagination and filters
    async fn list_with_filters(&self, params: CurrencyPaginationParams, filters: CurrencyFilter) -> Result<CurrencyPaginatedResult>;

    /// Count all currency entities
    async fn count(&self) -> Result<u64>;

    /// Count currency entities matching filters
    async fn count_with_filters(&self, filters: CurrencyFilter) -> Result<u64>;

    /// Check if currency exists by ID
    async fn exists(&self, id: &str) -> Result<bool>;

    // =========================================================================
    // Soft Delete Operations
    // =========================================================================

    /// Soft delete currency by ID
    async fn soft_delete(&self, id: &str) -> Result<bool>;

    /// Restore soft-deleted currency by ID
    async fn restore(&self, id: &str) -> Result<Option<Currency>>;

    /// List soft-deleted currency entities
    async fn list_deleted(&self, params: CurrencyPaginationParams) -> Result<CurrencyPaginatedResult>;

    /// Empty trash (permanently delete all soft-deleted entities)
    async fn empty_trash(&self) -> Result<u64>;

    // =========================================================================
    // Bulk Operations
    // =========================================================================

    /// Bulk save currency entities
    async fn bulk_save(&self, entities: &[Currency]) -> Result<Vec<Currency>>;

    /// Bulk delete by IDs
    async fn bulk_delete(&self, ids: &[&str]) -> Result<u64>;
}
