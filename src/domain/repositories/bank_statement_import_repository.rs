//! Repository trait for BankStatementImport aggregate
//!
...
pub struct BankStatementImportPaginationParams {
    pub page: u32,
    pub per_page: u32,
}

impl BankStatementImportPaginationParams {
...
#[derive(Debug, Clone)]
pub struct BankStatementImportPaginatedResult {
    pub data: Vec<BankStatementImport>,
    pub total: u64,
    pub page: u32,
    pub per_page: u32,
}
...
/// Repository trait for BankStatementImport aggregate
///
/// This trait defines the persistence contract for BankStatementImport entities.
...
pub trait BankStatementImportRepository: Send + Sync {
    ...
    /// Save a new bank_statement_import entity
    async fn save(&self, entity: &BankStatementImport) -> Result<BankStatementImport>;

    /// Find bank_statement_import by ID
    async fn find_by_id(&self, id: &str) -> Result<Option<BankStatementImport>>;

    /// Find all bank_statement_import entities
    ...
    /// Count bank_statement_import entities matching filters
    async fn count_with_filters(&self, filters: BankStatementImportFilter) -> Result<u64>;

    /// Check if bank_statement_import exists by ID
    ...
    /// Soft delete bank_statement_import by ID
    async fn soft_delete(&self, id: &str) -> Result<bool>;

    /// Restore soft-deleted bank_statement_import by ID
