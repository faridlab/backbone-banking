//! Application service for BankReconciliation entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `bank_reconciliation_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateBankReconciliationDto, UpdateBankReconciliationDto};
use crate::domain::entity::BankReconciliation;
use crate::infrastructure::persistence::BankReconciliationRepository;

/// Application service for BankReconciliation entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `bank_reconciliation_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct BankReconciliationServiceCustom {
///     inner: Arc<BankReconciliationService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type BankReconciliationService = GenericCrudService<
    BankReconciliation,
    CreateBankReconciliationDto,
    UpdateBankReconciliationDto,
    BankReconciliationRepository,
>;

// <<< CUSTOM
// END CUSTOM
