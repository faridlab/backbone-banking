...
use backbone_banking::application::service::banking_gl::{
    ...
    GlPostSink as BankSink, ReconcileEdgeAck, ReconcileLine, ReconcileOrigin, ReconcilePairRequest,
    ReconcileRejected, ReconcileSink, UnreconcilePairRequest,
};
use backbone_banking::application::service::banking_write_service::{
    BankingError, BankingWriteService, MatchCandidate, NewBank, NewBankAccount, NewClearance,
...
impl BankSink for GlAdapter {
    async fn post(&self, e: &BankEnv) -> Result<BankAck, BankRej> {
        ...
        match self
            ...
            .await
        {
            Ok((post_id, journal_id, idempotent_reuse)) => Ok(BankAck {
                post_id,
                journal_id,
                idempotent_reuse,
            }),
...
struct RecordingBankSink {
    events: Arc<Mutex<Vec<BankingEvent>>>,
}
impl BankingEventSink for RecordingBankSink {
...
async fn clearing_nets_undeposited_funds_across_three_modules() {
    ...
    let pay = payment
        .create_payment(NewPayment {
            ...
            bank_account_id: coa["1190"],
            party_account_id: coa["1200"],
            paid_amount: d("750000"),
            reference_no: Some(va_ref.clone()),
            allocations: vec![],
    ...
    assert_eq!(
        ...
        d("750000.00"),
        "clearing holds undeposited funds"
    );
    ...
    let acct = banking
        .create_bank_account(NewBankAccount {
            ...
            currency: None,
            account_type: Some("virtual_account".into()),
        })
        .await
        .unwrap();
    let import_id = banking
        .import_statement(NewStatementImport {
            ...
            lines: vec![NewStatementLine {
                txn_date: day(6),
                description: Some("Incoming VA".into()),
                reference_no: Some(va_ref.clone()),
                deposit: d("750000"),
    ...
    let cand = MatchCandidate {
        ...
        source_id: pay,
        amount: d("750000"),
        reference: Some(va_ref),
    };
    let matched = banking
        .propose_match(txn, &[cand])
        .await
        .unwrap()
        .expect("matched the payment");
    ...
    let out = banking
        .clear_transaction(
            NewClearance {
                ...
                matched_source_type: matched.source_type,
                matched_source_id: matched.source_id,
                matched_source_amount: matched.amount,
    ...
    assert_eq!(
        balance(&pool, coa["1190"]).await,
        d("0.00"),
        "clearing nets to zero — funds no longer undeposited"
    );
    assert_eq!(
        balance(&pool, coa["1110"]).await,
        d("750000.00"),
        "the bank GL holds the cash"
    );
    assert_eq!(
        balance(&pool, coa["1200"]).await,
        d("-750000.00"),
        "A/R stays settled (customer paid)"
    );
...
async fn two_lines(
    ...
    let ids: Vec<Uuid> = sqlx::query_scalar(
        "SELECT id FROM banking.bank_transactions WHERE import_id=$1 ORDER BY deposit DESC",
    )
    .bind(import_id)
    .fetch_all(pool)
    .await
...
async fn a_settlement_cannot_be_cleared_twice() {
    ...
    banking
        .clear_transaction(
            NewClearance {
                bank_transaction_id: l1,
                matched_source_type: "payment".into(),
                matched_source_id: pay,
    ...
    assert_eq!(
        balance(&pool, coa["1190"]).await,
        d("0.00"),
        "clearing stays at zero — no phantom credit"
    );
    assert_eq!(
        balance(&pool, coa["1110"]).await,
        d("500000.00"),
...
async fn one_settlement_splits_across_two_lines() {
    ...
    assert_eq!(
        balance(&pool, coa["1190"]).await,
        d("0.00"),
        "clearing nets to zero across the split"
    );
    assert_eq!(
        balance(&pool, coa["1110"]).await,
        d("750000.00"),
        "bank GL holds the full payment"
    );
...
async fn a_refused_clearing_edge_rolls_the_clear_back() {
    ...
    let e = banking
        .clear_transaction(
            NewClearance {
                ...
                matched_source_type: "payment".into(),
                matched_source_id: pay,
                matched_source_amount: d("400000"),
                matched_amount: d("400000"),
                match_method: None,
                clearance_date: day(6),
            },
            &gl,
        ...
        .unwrap_err();
    assert!(
        matches!(&e, BankingError::ReconcileRefused { code, .. } if code == "account_not_reconcilable"),
    ...
    assert_eq!(
        balance(&pool, coa["1110"]).await,
        d("400000.00"),
        "bank holds the money ONCE, not doubled"
    );
    assert_eq!(
        balance(&pool, coa["1190"]).await,
        d("0.00"),
...
async fn paid_out_clearing_edges_the_mirror_direction() {
    ...
    let pay = payment
        .create_payment(NewPayment {
            ...
            provider_txn_id: None,
            bank_account_id: coa["1190"],
            party_account_id: coa["2100"],
            paid_amount: d("300000"),
            reference_no: None,
            allocations: vec![],
            withholding_amount: Decimal::ZERO,
            withholding_account_id: None,
    ...
    assert_eq!(
        balance(&pool, coa["1190"]).await,
        d("0.00"),
        "clearing nets to zero — the outflow left"
    );
    assert_eq!(
        balance(&pool, coa["1110"]).await,
        d("-300000.00"),
        "the bank GL shows the withdrawal"
    );
