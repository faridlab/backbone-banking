...
use backbone_banking::application::service::banking_write_service::{
    BankingError, BankingWriteService, MatchCandidate, NewBank, NewBankAccount, NewCharge,
    NewClearance, NewReconciliation, NewStatementImport, NewStatementLine,
};
...
#[derive(Default, Clone)]
struct Recorder {
    events: Arc<Mutex<Vec<BankingEvent>>>,
}
impl BankingEventSink for Recorder {
    fn publish(&self, e: BankingEvent) {
...
impl ReconcileSink for NoEdgeSink {
    async fn reconcile_pair_on(
        ...
        Err(ReconcileRejected {
            ...
            message: "a journal match must not write a reconciliation edge".into(),
        })
    }
    async fn unreconcile_pair_on(
        ...
        Err(ReconcileRejected {
            code: "unexpected_edge".into(),
            message: "a journal match must not touch the reconciliation graph".into(),
        })
    }
}
...
fn line(dep: &str, wd: &str, refno: Option<&str>) -> NewStatementLine {
    NewStatementLine {
        txn_date: day(5),
        description: None,
        reference_no: refno.map(|s| s.to_string()),
        deposit: d(dep),
        withdrawal: d(wd),
    }
}
...
async fn import_balance_continuity() {
    ...
    let ok = NewStatementImport {
        ...
        period_start: day(1),
        period_end: day(31),
        opening_balance: d("1000000"),
        closing_balance: d("1300000"),
        file_ref: None,
        lines: vec![line("500000", "0", None), line("0", "200000", None)],
    };
    let import_id = w.import_statement(ok).await.unwrap();
    let (rows, st): (i32, String) = sqlx::query_as(
        "SELECT row_count, status::text FROM banking.bank_statement_imports WHERE id=$1",
    )
    .bind(import_id)
    .fetch_one(&pool)
    .await
    ...
    assert!(rec
        ...
        .unwrap()
        .iter()
        .any(|e| matches!(e, BankingEvent::BankStatementImported(s) if s.import_id == import_id)));
    ...
    let bad = NewStatementImport {
        ...
        period_end: day(31),
        opening_balance: d("1000000"),
        closing_balance: d("9999999"),
        file_ref: None,
        lines: vec![line("500000", "0", None)],
    };
    assert!(matches!(
        w.import_statement(bad).await.unwrap_err(),
        BankingError::BalanceMismatch { .. }
    ));
    // empty statement
    ...
    assert!(matches!(
        w.import_statement(empty).await.unwrap_err(),
        BankingError::EmptyStatement
    ));
}
...
async fn propose_match_amount_and_reference() {
    ...
    let txn: Uuid =
        sqlx::query_scalar("SELECT id FROM banking.bank_transactions WHERE import_id=$1")
            ...
            .await
            .unwrap();
    let pay_a = Uuid::new_v4();
    let pay_b = Uuid::new_v4();
    let cands = vec![
        MatchCandidate {
...
async fn clear_deposit_posts_bank_over_clearing() {
    ...
    let out = w
        .clear_transaction(
            NewClearance {
                ...
                matched_source_type: "journal".into(),
                matched_source_id: Uuid::new_v4(),
                matched_source_amount: d("750000"),
                matched_amount: d("750000"),
    ...
    assert_eq!(
        env.lines
            .iter()
            .find(|l| l.account_id == clearing)
            .unwrap()
            .credit,
        d("750000.00")
    );
    let st: String =
        sqlx::query_scalar("SELECT status::text FROM banking.bank_transactions WHERE id=$1")
            .bind(txn)
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(st, "reconciled");
    assert!(rec.events.lock().unwrap().iter().any(
        |e| matches!(e, BankingEvent::BankTransactionCleared(c) if c.bank_transaction_id == txn)
    ));
}
...
async fn clear_withdrawal_and_partial() {
    ...
    let out = w
        .clear_transaction(
            NewClearance {
                ...
                matched_source_type: "journal".into(),
                matched_source_id: Uuid::new_v4(),
                matched_source_amount: d("250000"),
                matched_amount: d("250000"),
                match_method: None,
                clearance_date: day(6),
            },
            &gl,
    ...
    let (al, st): (Decimal, String) = sqlx::query_as(
...
async fn bank_charge_posts_expense_over_bank() {
    ...
    let import_id = w
        .import_statement(NewStatementImport {
            ...
            period_start: day(1),
            period_end: day(31),
            opening_balance: d("100000"),
            closing_balance: d("85000"),
            file_ref: None,
    ...
    let txn: Uuid =
        sqlx::query_scalar("SELECT id FROM banking.bank_transactions WHERE import_id=$1")
            ...
            .unwrap();
    let gl = FakeGl::default();
    w.recognize_bank_charge(
        NewCharge {
            bank_transaction_id: txn,
            charge_account_id: charges,
            amount: d("15000"),
            clearance_date: day(6),
        },
        &gl,
...
async fn reconcile_closes_when_balanced() {
    ...
    let out = w
        .reconcile(NewReconciliation {
            ...
            from_date: day(1),
            to_date: day(31),
            statement_closing_balance: d("1300000"),
            ledger_balance: d("1300000"),
        })
        .await
        .unwrap();
    ...
    assert!(rec
        ...
        .unwrap()
        .iter()
        .any(|e| matches!(e, BankingEvent::BankReconciliationClosed(_))));
...
async fn reconcile_with_open_lines_stays_balanced_not_closed() {
    ...
    let out = w
        .reconcile(NewReconciliation {
            ...
            from_date: day(1),
            to_date: day(31),
            statement_closing_balance: d("500000"),
            ledger_balance: d("500000"),
        })
        .await
        .unwrap();
    assert_eq!(out.difference, d("0.00"));
    ...
    assert!(
        !rec.events
            ...
            .iter()
            .any(|e| matches!(e, BankingEvent::BankReconciliationClosed(_))),
        "no false attestation"
    );
    let persisted: (String, i32) = sqlx::query_as(
        "SELECT status::text, unreconciled_count FROM banking.bank_reconciliations WHERE id=$1",
    )
    .bind(out.id)
    .fetch_one(&pool)
    .await
    ...
    assert_eq!(
        persisted,
        ("balanced".to_string(), 1),
        "exception snapshot is persisted"
    );
